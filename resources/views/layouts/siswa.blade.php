...
<head>
    ...
    <style>
        ...
        .sidebar-link::before {
            ...
            height: 100%;
            width: 4px;
            background: #eab308; /* Yellow for Siswa */
        ...
        .sidebar-link.active {
            background: linear-gradient(90deg, rgba(234, 179, 8, 0.15) 0%, rgba(234, 179, 8, 0) 100%);
            color: #facc15;
            font-weight: 600;
        }
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-72 bg-[#0f172a] text-slate-400 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl flex flex-col border-r border-slate-800">
        ...
        <div class="h-32 flex flex-col items-center justify-center px-6 border-b border-slate-800/50 bg-[#0f172a] relative z-10">
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-yellow-500 to-amber-600 flex items-center justify-center shadow-lg shadow-yellow-500/20 transform hover:rotate-6 transition-transform duration-300">
                    <i class="fas fa-user-graduate text-white text-xl"></i>
                </div>
                <div>
                    <span class="text-xl font-bold text-white tracking-tight block leading-none">SIAKAD</span>
                    <span class="text-[10px] text-yellow-500 font-bold uppercase tracking-[0.2em] mt-1 block">Siswa Panel</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar flex flex-col py-6">
            ...
            <nav class="flex-1 px-4 space-y-1.5">
                ...
                <a href="{{ route('siswa.dashboard') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('siswa.dashboard') ? 'active' : '' }}">
                    <i class="fas fa-home w-6 text-center group-hover:text-yellow-400"></i>
                ...
                <a href="{{ route('siswa.nilai.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('siswa.nilai.index') ? 'active' : '' }}">
                    ...
                    <span class="ml-3">Lihat Nilai</span>
                </a>
                <a href="{{ route('siswa.absensi.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('siswa.absensi.index') ? 'active' : '' }}">
                    <i class="fas fa-user-clock w-6 text-center group-hover:text-yellow-400"></i>
            ...
            <div class="px-4 mt-8 mb-4">
                <div class="p-4 rounded-2xl bg-slate-800/40 border border-slate-700/50 backdrop-blur-sm">
                    <div class="flex items-center mb-4">
                        <div class="relative">
                            <img class="h-10 w-10 rounded-full object-cover border-2 border-yellow-500 shadow-lg shadow-yellow-500/20" src="https://ui-avatars.com/api/?name={{ Auth::user()->name }}&background=eab308&color=fff" alt="Siswa Profile">
    ...
    <div class="lg:ml-72 flex flex-col min-h-screen transition-all duration-300">
        ...
        <header class="glass-header h-20 flex items-center justify-between px-6 md:px-10 sticky top-0 z-20">
            <div class="flex items-center">
                <button id="sidebarToggle" class="text-slate-500 hover:text-yellow-600 focus:outline-none lg:hidden mr-4 p-2.5 rounded-xl hover:bg-yellow-50 transition-all">
                    <i class="fas fa-bars-staggered text-xl"></i>
                </button>
                <div class="flex flex-col">
                    <h2 class="text-lg md:text-xl font-bold text-slate-800 leading-tight">@yield('header', 'Dashboard Siswa')</h2>
                    <p class="text-[10px] md:text-xs text-slate-400 font-medium hidden sm:block">Pantau perkembangan akademik Anda</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 md:gap-6">
                ...
                <div class="hidden md:flex flex-col items-end">
                    ...
                    <span class="text-[10px] text-yellow-600 font-bold bg-yellow-50 px-2 py-0.5 rounded-full uppercase tracking-wider">Tahun Ajaran 2025/2026</span>
                </div>
                ...
                <!-- Profile Mobile -->
                <div class="lg:hidden">
                    <img class="h-9 w-9 rounded-full border-2 border-yellow-500" src="https://ui-avatars.com/api/?name={{ Auth::user()->name }}&background=eab308&color=fff" alt="">
                </div>
            </div>
        </header>
