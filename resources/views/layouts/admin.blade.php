    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-72 bg-[#0f172a] text-slate-400 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl flex flex-col border-r border-slate-800">
        ...
        <div class="h-32 flex flex-col items-center justify-center px-6 border-b border-slate-800/50 bg-[#0f172a] relative z-10">
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/20 transform hover:rotate-6 transition-transform duration-300">
                    <i class="fas fa-graduation-cap text-white text-xl"></i>
                </div>
                <div>
                    <span class="text-xl font-bold text-white tracking-tight block leading-none">SIAKAD</span>
                    <span class="text-[10px] text-blue-500 font-bold uppercase tracking-[0.2em] mt-1 block">Admin Panel</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar flex flex-col py-6">
            ...
            <nav class="flex-1 px-4 space-y-1.5">
                ...
                <a href="{{ route('admin.dashboard') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
                    <i class="fas fa-home w-6 text-center group-hover:text-blue-400"></i>
                ...
                <a href="{{ route('admin.guru.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.guru.*') ? 'active' : '' }}">
                    <i class="fas fa-chalkboard-teacher w-6 text-center group-hover:text-blue-400"></i>
                    <span class="ml-3">Data Guru</span>
                </a>
                <a href="{{ route('admin.siswa.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.siswa.*') ? 'active' : '' }}">
                    <i class="fas fa-user-graduate w-6 text-center group-hover:text-blue-400"></i>
                    <span class="ml-3">Data Siswa</span>
                </a>
                <a href="{{ route('admin.kelas.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.kelas.*') ? 'active' : '' }}">
                    <i class="fas fa-school w-6 text-center group-hover:text-blue-400"></i>
                    <span class="ml-3">Data Kelas</span>
                </a>
                <a href="{{ route('admin.mapel.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.mapel.*') ? 'active' : '' }}">
                    <i class="fas fa-book w-6 text-center group-hover:text-blue-400"></i>
                ...
                <a href="{{ route('admin.jadwal.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.jadwal.*') ? 'active' : '' }}">
                    <i class="fas fa-calendar-alt w-6 text-center group-hover:text-blue-400"></i>
                    <span class="ml-3">Jadwal Pelajaran</span>
                </a>
                <a href="{{ route('admin.nilai.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.nilai.*') ? 'active' : '' }}">
                    <i class="fas fa-file-invoice w-6 text-center group-hover:text-blue-400"></i>
                    <span class="ml-3">Nilai & Raport</span>
                </a>
                <a href="{{ route('admin.absensi.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.absensi.*') ? 'active' : '' }}">
                    <i class="fas fa-user-clock w-6 text-center group-hover:text-blue-400"></i>
                ...
                <a href="{{ route('admin.profile.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-800/50 hover:text-white group {{ request()->routeIs('admin.profile.*') ? 'active' : '' }}">
                    <i class="fas fa-cog w-6 text-center group-hover:text-blue-400"></i>
                    <span class="ml-3">Konfigurasi Akun</span>
                </a>
            </nav>
            ...
            <div class="px-4 mt-8 mb-4">
                <div class="p-4 rounded-2xl bg-slate-800/40 border border-slate-700/50 backdrop-blur-sm">
                    <div class="flex items-center mb-4">
                        <div class="relative">
                            <img class="h-10 w-10 rounded-full object-cover border-2 border-blue-500 shadow-lg shadow-blue-500/20" src="https://ui-avatars.com/api/?name={{ Auth::user()->name }}&background=3b82f6&color=fff" alt="Admin Profile">
    ...
    <div class="lg:ml-72 flex flex-col min-h-screen transition-all duration-300">
        ...
        <header class="glass-header h-20 flex items-center justify-between px-6 md:px-10 sticky top-0 z-20">
            <div class="flex items-center">
                <button id="sidebarToggle" class="text-slate-500 hover:text-blue-600 focus:outline-none lg:hidden mr-4 p-2.5 rounded-xl hover:bg-blue-50 transition-all">
                    <i class="fas fa-bars-staggered text-xl"></i>
                </button>
                <div class="flex flex-col">
                    <h2 class="text-lg md:text-xl font-bold text-slate-800 leading-tight">@yield('header', 'Dashboard')</h2>
                    <p class="text-[10px] md:text-xs text-slate-400 font-medium hidden sm:block">Selamat datang kembali di panel administrasi</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 md:gap-6">
                ...
                <div class="hidden md:flex flex-col items-end">
                    ...
                    <span class="text-[10px] text-blue-600 font-bold bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-wider">Terverifikasi</span>
                </div>
                ...
                <!-- Profile Mobile -->
                <div class="lg:hidden">
                    <img class="h-9 w-9 rounded-full border-2 border-blue-500" src="https://ui-avatars.com/api/?name={{ Auth::user()->name }}&background=3b82f6&color=fff" alt="">
                </div>
            </div>
        </header>
